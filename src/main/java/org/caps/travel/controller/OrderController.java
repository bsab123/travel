...
import org.caps.travel.utils.PaymentUtil;
...
public class OrderController {
    ...
    public void payForOrder(HttpServletRequest request, HttpServletResponse response) {
        // 获得回调所有数据
        String p1_MerId = request.getParameter(this.p1_MerId);
        ...
        if (isValid) {
            try {
                ...
                if (r9_BType.equals("1")) {
                    ...
                    // 浏览器重定向（展示给客户端的界面）
                    response.sendRedirect("http://localhost:8080/paySuccessPage?no="+r6_Order+"&code="+order.getCode());
